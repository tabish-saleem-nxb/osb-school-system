...
#  # Load invoice line items data when an item is selected from drop down list
  ...
  def item_params
    params.require(:item).permit(:inventory, :item_description, :item_name, :item_ids, :quantity, :tax_1, :tax_2, :track_inventory, :unit_cost, :archive_number, :archived_at, :deleted_at, :grade_id, :item_type)
  end

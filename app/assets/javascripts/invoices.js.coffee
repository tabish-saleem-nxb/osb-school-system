...
window.applyChosen = (single_row) =>
  ...
  jQuery(".chzn-select:not('.remove-add-new, .invoice_company, .company_filter, .frequency_select, .per_page, .discount_select, .invoice_currency, .recurring_profile_currency, .report_billing_method, .expense_category')",'.estimates-main, .projects-main, .invoices-main,.recurring_profiles-main, .expenses-main').on "liszt:ready", ->
...
#    jQuery("#invoice_discount_amount_lbl").text(discount_amount.toFixed(2))
  ...
  jQuery("#invoice_invoice_date, #invoice_due_date, .date_picker_class, #penalty_rule_due_date, #client_fee_date").datepicker
